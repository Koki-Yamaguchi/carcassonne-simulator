    ...
    for (let x = 1; x < boardSize - 1; x ++) {
      if (tiles.value[y][x] === null) {
        ...
        for (let dir = 0; dir < 4; dir ++) {
          ...
          if ((tiles.value[y - 1][x] !== null && tiles.value[y - 1][x]?.Bottom() !== placingTile.value.Top()) ||
            (tiles.value[y + 1][x] !== null && tiles.value[y + 1][x]?.Top() !== placingTile.value.Bottom()) ||
            (tiles.value[y][x - 1] !== null && tiles.value[y][x - 1]?.Right() !== placingTile.value.Left()) ||
            (tiles.value[y][x + 1] !== null && tiles.value[y][x + 1]?.Left() !== placingTile.value.Right())) {
            continue;
          }
...
const handlePositionSelected = (pos: [number, number]) => {
  ...
  while (!placeableDirections.value.includes(placingTile.value?.Direction)) {
    placingTile.value?.Rotate()
  }
...
const confirm = () => {
  tiles.value[placingPosition.value[0]][placingPosition.value[1]] = placingTile.value
